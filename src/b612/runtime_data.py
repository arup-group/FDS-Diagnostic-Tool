...
import b612.scraping_fxn as scr
import pandas as pd
import time
import json
import os
...
def get_data(outfile_file_path, output_loc, config, mesh_data):
    ...
    sim_info['date_start'] = sim_info['date_start'].strftime("%d/%m/%Y %H:%M:%S")
    sim_info['lst_sim_time'] = per_cycle_info['lst']['sim_time'].iloc[-1]
    sim_info['lst_log_time'] = per_cycle_info['lst']['log_time'].iloc[-1].strftime("%d/%m/%Y %H:%M:%S")
